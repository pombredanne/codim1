...
def test_degree_zero():
    ...
    for x in np.linspace(0.0, 1.0, 10):
        ...
        np.testing.assert_almost_equal(
                bf.derivs.evaluate(0, 0, x, x), 0.0)
...
def test_basis_derivative():
    ...
    yd_exact2 = -2 * x_hat
    yd_exact3 = x_hat + 0.5
    yd_est1 = derivs.evaluate(0, 0, x_hat, x_hat)
    yd_est2 = derivs.evaluate(0, 1, x_hat, x_hat)
    yd_est3 = derivs.evaluate(0, 2, x_hat, x_hat)
...
def test_derivative_chainrule():
    ...
    derivs = bf.derivs
    assert(derivs.chain_rule(0) == 0.5)
