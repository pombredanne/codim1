...
def sgbem_interior(mesh, pt, normal, kernel_set, bc_type):
    i = InteriorPoint()
    which_kernels = _make_which_kernels(kernel_set)
    for e_l in mesh:
        # Handle the boundary condition first
        _interior_element(i, pt, normal, bc_type, e_l, which_kernels, "bc")
        _interior_element(i, pt, normal, bc_type, e_l, which_kernels, "soln")
    return np.array(i.result)

def _interior_element(integrator, pt, normal, bc_type,
    ...
    kernel, factor = which_kernels[bc_type][e_l_type]["interior"]
    if kernel is None:
        return
